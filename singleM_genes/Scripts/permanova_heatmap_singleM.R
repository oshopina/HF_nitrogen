...
cluster_table = read.csv2('singleM_genes/Results/permanova_by_hellinger_res.csv', row.names = 1)
cluster_table$Gene[cluster_table$Gene == 'narG'] = 'narG_nxrA'
cluster_table$Gene[cluster_table$Gene == 'narH'] = 'narH_nxrB'

heatmaps = list()
for (i in colnames(cluster_table[,1:2])) {
  ...
  plot = Heatmap(t(heatmap_table), cluster_rows = F, cluster_columns = F, row_order = gene_order$Gene,
                 column_order = c(unique(cluster_table$cluster)[2:51], (unique(cluster_table$cluster)[1])),
